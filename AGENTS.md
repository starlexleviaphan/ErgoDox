...
- **Автоматический скрипт валидации**: 
  ...
  ```
- **Автоматический скрипт сборки и загрузки прошивки**:
  ```bash
  node .agents/skills/firmware-critic/scripts/build.js
  ```
  Коммитит изменения в `zmk-config`, запускает сборку на GitHub Actions, отслеживает статус и автоматически скачивает скомпилированные бинарники (`.uf2`) в папку `firmware/`.
...
- **Проверяемые инварианты**:
  ...
  - **Каталог прошивок**: Все свежие бинарники (`.uf2`) должны всегда помещаться напрямую в папку `firmware/` репозитория.

## Правила репозитория (`.agents/rules/`)
- `ergodox_sync.md`: Обязательная 100% взаимная синхронизация между `layout-viewer/data.js`, `ergodox.keymap` и `docs/layout/02_LAYERS.md` при любых изменениях раскладки, а также автоматическая сборка и размещение готовых `.uf2` в `firmware/`.
