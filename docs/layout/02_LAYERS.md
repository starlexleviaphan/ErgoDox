...
Включается тапом по правому Shift на базовом слое (`WORK Shift` / `Key 64`). Выход обратно в WORK — повторный тап по той же клавише.

### Назначение
- **Прозрачность большинства клавиш (`&trans`)**: 56 клавиш из 76 прозрачны (`▽`) и напрямую пробрасывают нажатия на базовый слой `WORK (0)`. На слое `TYPE` нет дублирования альфа-символов, цифр или служебных кнопок.
- **Снятие модификаторов с домашнего ряда**: переопределены только клавиши `A, S, D, F` (левая рука) и `J, K, L, ;` (правая рука) — на них отключены hold-модификаторы, превращая их в 100% чистые символы для сверхбыстрой печати без случайных ложных срабатываний хоткеев.
- **Навигация**:
  - `PgUp` (`Key 13`) и `PgDn` (`Key 26`) под левой рукой.
  - Стрелки `↑` (`Key 33`) и `↓` (`Key 34`) на левом большом пальце для редактирования текста на лету.
- **Управление AutoShift** (опционально):
  - `AS_TOGG`: мгновенное включение/отключение AutoShift.
  - `AS_RPT`: повтор последнего введенного символа.
  - `AS_UP` / `AS_DOWN`: подстройка задержки AutoShift.
